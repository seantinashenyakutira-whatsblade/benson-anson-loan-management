-- Phase 5, Migration 032: Payment trigger → notification_queue
-- Replaces rpc_record_payment to queue a payment_received email for the customer.
...
BEGIN
  ...
  SELECT outstanding_balance INTO v_new_balance FROM loans WHERE id = p_loan_id;
  ...
  -- Queue payment_received email (best-effort, must not block payment)
  BEGIN
    INSERT INTO notification_queue (recipient_customer_id, recipient_email, channel, template, payload)
    SELECT
      v_customer_id, c.email, 'email', 'payment_received',
      jsonb_build_object(
        'customer_name', COALESCE(c.first_name || ' ' || c.last_name, c.first_name, 'Customer'),
        'loan_no', v_loan_number,
        'amount', p_amount,
        'method', p_method,
        'receipt_no', v_payment_number,
